...
export default function DiscoverClubsPage() {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8 max-w-7xl">
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-xl p-6 sm:p-8 shadow-2xl border border-white/60 transition-all duration-300 hover:bg-white/85">
          ...
          >
            ...
            <p className="text-gray-700 mt-2 text-lg sm:text-xl max-w-2xl mx-auto">
      ...
      {error && (
        <div className="text-center text-red-600 py-6 bg-red-50 backdrop-blur-md rounded-lg mb-8 p-6 mx-4 sm:mx-0 border border-red-200">
          <p className="font-medium text-lg text-red-700">⚠️ Error loading clubs</p>
          <p className="text-sm mt-2 text-red-600">{error}</p>
        </div>
      )}
      
      {!loading && clubs.length === 0 && (
        <div className="text-center py-12 bg-gray-50 backdrop-blur-md rounded-lg shadow-md border border-gray-200">
          <Building2 className="h-16 w-16 text-gray-600 mx-auto mb-6" />
          <p className="text-gray-700 text-lg">🎭 No clubs found. The stage is waiting to be set!</p>
          <p className="text-gray-600 text-sm mt-2">The database might need to be seeded with club data.</p>
        </div>
      )}
      ...
      {clubs.length > 0 && (
        ...
        >
          {clubs.map((club, index) => (
            ...
            >
              <Card className="h-full flex flex-col hover:shadow-lg transition-all duration-300 bg-white/95 backdrop-blur-sm text-gray-900 rounded-lg shadow-md border border-gray-200 hover:border-gray-300 relative overflow-hidden group transform hover:scale-105 hover:shadow-xl hover:shadow-gray-200">
                {club.logoUrl && (
                  <div className="aspect-video w-full overflow-hidden rounded-t-lg bg-gradient-to-br from-gray-50 to-gray-100 relative">
                    <img
                      src={club.logoUrl}
                      alt={`${club.name} logo`}
                ...
                <CardHeader className="pb-3">
                  <CardTitle className="text-base sm:text-lg leading-tight text-gray-900 group-hover:text-gray-700 transition-colors">
                    {club.name}
                  </CardTitle>
                ...
                <CardContent className="flex-grow flex flex-col justify-between pt-0">
                  ...
                  >
                    <Button
                      ...
                      className="w-full text-sm bg-blue-600 hover:bg-blue-700 text-white font-semibold shadow-lg hover:shadow-xl transition-all duration-300"
                      size="sm"
                    >
                      {status !== "authenticated"
                        ? "🔐 Sign in to Join"
                ...
                {/* Subtle glow effect */}
                <div className="absolute inset-0 rounded-lg opacity-0 group-hover:opacity-30 transition-opacity duration-300 bg-gradient-to-r from-blue-50/10 via-blue-50/5 to-blue-50/10 pointer-events-none"></div>
              </Card>
            </motion.div>
          ))}
        </motion.div>
